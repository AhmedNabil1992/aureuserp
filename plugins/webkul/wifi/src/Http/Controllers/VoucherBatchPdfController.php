...
class VoucherBatchPdfController extends Controller
{
    public function download(string $batchCode): Response
    {
        try {
            ...
            $cacheDirectory = $this->resolveWritableTcpdfCacheDirectory();

            if (! defined('K_PATH_CACHE')) {
                define('K_PATH_CACHE', rtrim($cacheDirectory, DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR);
            }
            ...
            return response()->json([
                'message' => 'تعذر تنزيل ملف الكروت. تحقق من صلاحيات الكتابة على storage لدى IIS.',
            ], 500);
        }
    }
    ...
    private function resolveWritableTcpdfCacheDirectory(): string
    {
        $candidateDirectories = [
            storage_path('app/tcpdf_temp'),
            storage_path('framework/cache/tcpdf'),
            rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.'tcpdf',
        ];
        ...
        throw new \RuntimeException('No writable TCPDF cache directory found for current server user.');
    }
}
