...
class FindMissingTranslations extends Command
{
    ...
    {
        return array_keys(config('app.supported_locales', ['en' => [], 'ar' => []]));
    }
    ...
    protected function processLangFolder(string $name, string $langRoot, ?string $targetLocale): void
    {
        ...
        $enFilesRel = $enFiles->map(fn ($f) => $this->toRelativePath($f, $enPath));
    ...
    protected function checkLocale(
        ...
        $localeFilesRel = $localeFiles->map(fn ($f) => $this->toRelativePath($f, $localePath));
