...
def _detect_fw(root):
    for sub in ['', 'backend', 'api', 'server', 'app']:
        comp = os.path.join(root, sub, 'composer.json') if sub else os.path.join(root, 'composer.json')
        art = os.path.join(root, sub, 'artisan') if sub else os.path.join(root, 'artisan')
        if os.path.isfile(comp) or os.path.isfile(art):
            return 'laravel'
...
def _scan_laravel(root):
    plugins_dirs = [
        os.path.join(root, 'plugins', 'webkul'),
        os.path.join(root, 'plugins'),
    ]
    plugins_dir = None
    for pd in plugins_dirs:
        if os.path.isdir(pd) and any(os.path.isdir(os.path.join(pd, sub)) for sub in os.listdir(pd)):
            plugins_dir = pd
            break
    ...
    if plugins_dir:
        plugin_folders = sorted([f for f in os.listdir(plugins_dir) if os.path.isdir(os.path.join(plugins_dir, f))])
    else:
        plugin_folders = []

    for idx, pfolder in enumerate(plugin_folders):
        pdir = os.path.join(plugins_dir, pfolder)
        raw_name = pfolder
        name = raw_name.replace('-', ' ').title()
        mid = raw_name.lower().replace('-', '_')

        api_routes_file = os.path.join(pdir, 'routes', 'api.php')
        web_routes_file = os.path.join(pdir, 'routes', 'web.php')

        base_path = f"/api/v1/{raw_name}"
        eps = []
        seen = set()

        files = []
        for r, _, fls in os.walk(pdir):
            norm_r = r.replace('\\', '/')
            if any(x in norm_r for x in ['/vendor/', '/node_modules/', '/tests/', '/database/']): continue
            for f in fls:
                if f.endswith('.php'):
                    files.append(os.path.relpath(os.path.join(r, f), root).replace('\\', '/'))

        # Parse routes/api.php
        if os.path.isfile(api_routes_file):
            txt = open(api_routes_file, encoding='utf-8', errors='ignore').read()

            pfx_m = re.search(r"->prefix\(['\"]([^'\"]+)['\"]\)", txt)
            if pfx_m:
                base_path = '/' + pfx_m.group(1).strip('/')

            global_auth = ('auth:sanctum' in txt or 'auth' in txt)

            # softDeletableApiResource
            for m in re.finditer(r"Route::softDeletableApiResource\(['\"]([^'\"]+)['\"]\s*,\s*(\w+Controller)::class\)", txt):
                res_name, ctrl = m.group(1), m.group(2)
                res_singular = res_name.rstrip('s')
                crud_eps = [
                    ('GET', f"/{res_name}", True, f"{mid}.{res_name}.view", f"List all {res_name}"),
                    ('POST', f"/{res_name}", True, f"{mid}.{res_name}.create", f"Create {res_singular}"),
                    ('GET', f"/{res_name}/{{id}}", True, f"{mid}.{res_name}.view", f"Get {res_singular} details"),
                    ('PUT', f"/{res_name}/{{id}}", True, f"{mid}.{res_name}.update", f"Update {res_singular}"),
                    ('DELETE', f"/{res_name}/{{id}}", True, f"{mid}.{res_name}.delete", f"Delete {res_singular}"),
                    ('PUT', f"/{res_name}/{{id}}/restore", True, f"{mid}.{res_name}.restore", f"Restore deleted {res_singular}"),
                    ('DELETE', f"/{res_name}/{{id}}/force", True, f"{mid}.{res_name}.force-delete", f"Permanently delete {res_singular}"),
                ]
                for meth, pth, au, pr, ds in crud_eps:
                    key = f"{meth}:{pth}"
                    if key not in seen:
                        seen.add(key)
                        eps.append({'method': meth, 'path': pth, 'auth': au, 'permission': pr, 'description': ds})

            # apiResource
            for m in re.finditer(r"Route::apiResource\(['\"]([^'\"]+)['\"]\s*,\s*(\w+Controller)::class\)", txt):
                res_name, ctrl = m.group(1), m.group(2)
                res_singular = res_name.rstrip('s')
                crud_eps = [
                    ('GET', f"/{res_name}", True, f"{mid}.{res_name}.view", f"List all {res_name}"),
            ...
            # Standard routes
            for m in re.finditer(r"Route::(get|post|put|patch|delete)\s*\(\s*['\"]([^'\"]*)['\"]", txt, re.IGNORECASE):
                method, path = m.group(1).upper(), m.group(2)
                if not path.startswith('/'): path = '/' + path
                key = f"{method}:{path}"
                if key not in seen:
                    seen.add(key)
                    eps.append({'method': method, 'path': path, 'auth': global_auth, 'permission': f"{mid}.access", 'description': _infer_desc(method, path, name)})

        # Parse routes/web.php
        if os.path.isfile(web_routes_file):
            txt = open(web_routes_file, encoding='utf-8', errors='ignore').read()
            for m in re.finditer(r"Route::(get|post|put|patch|delete)\s*\(\s*['\"]([^'\"]*)['\"]", txt, re.IGNORECASE):
                method, path = m.group(1).upper(), m.group(2)
                if not path.startswith('/'): path = '/' + path
                key = f"{method}:{path}"
                if key not in seen:
                    seen.add(key)
                    eps.append({'method': method, 'path': path, 'auth': False, 'permission': None, 'description': f"Web page route — {path}"})

        # If no endpoints found from routes, add default panel endpoint for the plugin
        if not eps:
            eps.append({
                'method': 'GET',
                'path': f"/admin/{raw_name}",
                'auth': True,
                'permission': f"{mid}.access",
                'description': f"Filament Admin UI Panel — {name}"
            })
        ...
        modules.append({
            ...
            'name': name,
            'basePath': base_path,
            'description': f"Webkul {name} Plugin Module — {len(eps)} endpoint(s) & Filament UI components",
            'color': _color(raw_name, idx),
            'icon': _icon(raw_name),
            'files': files[:15],
            'permissions': perms,
            'endpoints': eps
    ...
    # Also scan app/ directory for root modules
    app_dir = os.path.join(root, 'app')
    if os.path.isdir(app_dir):
        app_files = []
        for r, _, fls in os.walk(app_dir):
            norm_r = r.replace('\\', '/')
            for f in fls:
                if f.endswith('.php'):
                    app_files.append(os.path.relpath(os.path.join(r, f), root).replace('\\', '/'))
        if app_files:
            modules.append({
                'id': 'app_core',
                'name': 'App Core',
                'basePath': '/admin',
                'description': 'AureusERP Core Application, Filament Providers, Models & Middleware',
                'color': '#6366f1',
                'icon': '⚡',
                'files': app_files[:15],
                'permissions': ['admin.access', 'customer.access'],
                'endpoints': [
                    {'method': 'GET', 'path': '/admin', 'auth': True, 'permission': 'admin.access', 'description': 'Filament Admin Panel Dashboard'},
                    {'method': 'GET', 'path': '/portal', 'auth': True, 'permission': 'customer.access', 'description': 'Filament Customer Portal Dashboard'},
                    {'method': 'GET', 'path': '/login', 'auth': False, 'permission': None, 'description': 'Authentication Redirect & Login Landing'}
                ]
            })
...
def _scan_workspaces(root):
    if os.path.isfile(os.path.join(root, 'composer.json')):
        return [
            {'id': 'admin_panel', 'name': 'Admin Panel (/admin)', 'type': 'backend', 'description': 'Filament v5 Enterprise ERP Admin Panel', 'port': 80, 'entrypoint': 'app/Providers/Filament/AdminPanelProvider.php'},
            {'id': 'customer_panel', 'name': 'Customer Portal (/portal)', 'type': 'frontend', 'description': 'Filament v5 Customer Self-Service Portal', 'port': 80, 'entrypoint': 'app/Providers/Filament/CustomerPanelProvider.php'},
            {'id': 'website_panel', 'name': 'Public Website (/)', 'type': 'frontend', 'description': 'Public CMS & Website Landing Panel', 'port': 80, 'entrypoint': 'app/Providers/Filament/WebsitePanelProvider.php'},
            {'id': 'rest_api', 'name': 'REST API Gateway (/api/v1)', 'type': 'backend', 'description': 'Sanctum-Authenticated Plugin REST API', 'port': 80, 'entrypoint': 'plugins/webkul/*/routes/api.php'}
        ]
    ws = []
    pom_path = os.path.join(root, 'pom.xml')
    if os.path.isfile(pom_path):
...
def _scan_core_layer(root, fw):
    ...
    if fw == 'laravel':
        sec = [
            {"name": "Laravel Sanctum", "file": "composer.json", "description": "API Token & SPA Cookie-based authentication framework"},
            {"name": "Filament Shield", "file": "composer.json", "description": "Spatie Permission-based Role Access Control (RBAC) & Panel Authorization"},
            {"name": "CSRF & Security Middleware", "file": "app/Http/Middleware/", "description": "Cross-Site Request Forgery protection & encrypted session state"},
            {"name": "Bcrypt & Argon2 Hashes", "file": "config/hashing.php", "description": "Secure password hashing and credential verification"}
        ]
        mid = [
            {"name": "Authenticate (Sanctum/Session)", "file": "app/Http/Middleware/Authenticate.php", "description": "Guards incoming HTTP requests for Admin/Customer panels and API endpoints", "guards": ["Auth Guard"]},
            {"name": "Spatie Permission Middleware", "file": "vendor/spatie/laravel-permission", "description": "Enforces permission slugs and role checks on routes and Livewire actions", "guards": ["RBAC Guard"]},
            {"name": "Filament Panel Middleware", "file": "app/Providers/Filament/", "description": "Initializes Filament v5 theme context, tenant state, and panel assets", "guards": ["UI Context"]}
        ]
        svc = [
            {"name": "Webkul Plugin Manager", "file": "plugins/webkul/plugin-manager", "description": "Discovers, registers, seeds, and manages plugin lifecycle", "exports": ["PluginManagerServiceProvider"]},
            {"name": "Livewire v4 Engine", "file": "composer.json", "description": "Full-stack reactive component rendering engine for Filament v5 UI", "exports": ["Livewire"]},
            {"name": "Spatie QueryBuilder", "file": "composer.json", "description": "Filters, includes, sorts, and paginates Eloquent query results for API endpoints", "exports": ["QueryBuilder"]},
            {"name": "Laravel Reverb WebSockets", "file": "composer.json", "description": "High-performance WebSocket server for real-time notifications and chatter", "exports": ["Reverb"]},
            {"name": "Barryvdh DOMPDF Engine", "file": "composer.json", "description": "Renders PDF invoices, reports, and purchase orders from Blade templates", "exports": ["PdfFacade"]}
        ]
...
def _build_data_flow(fw, core_layer):
    ...
    if fw == 'laravel':
        pipeline = [
            {"step": "HTTP request received by Nginx / Swoole / FrankenPHP / Sail", "detail": "Web server routes all requests to public/index.php bootstrap file."},
            {"step": "Laravel Kernel bootstraps application providers & middleware stack", "detail": "Registers service providers, loads .env config, and initializes HTTP Kernel.", "coreRef": "middleware"},
            {"step": "Route matched by Laravel Router (Web / API / Filament Panel)", "detail": "Route pattern matched against URL parameters. Controller or Livewire component resolved."},
            {"step": "Sanctum / Session auth middleware verifies identity", "detail": "Auth guard checks Bearer token (API) or session cookie (Admin/Customer Panel). 401 on failure.", "coreRef": "security"},
            {"step": "Filament Shield / Policy Gate checks user authorization", "detail": "@can or Spatie permission checked against user roles. 403 Forbidden on failure.", "coreRef": "security"},
            {"step": "FormRequest / Livewire validation rules executed", "detail": "Incoming payload validated against rules. Fails with 422 Unprocessable Content if invalid.", "coreRef": "services"},
            {"step": "Controller / Livewire action executes business logic", "detail": "Service logic invoked. Model events and database transactions triggered.", "coreRef": "services"},
            {"step": "Eloquent ORM / Spatie QueryBuilder executes database queries", "detail": "SQL queries generated with eager loading. Query results mapped to Model instances.", "coreRef": "services"},
            {"step": "Response formatted — Blade/Livewire DOM update or JSON Resource returned", "detail": "API Resource serializes model to JSON (200 OK) or Livewire updates reactive DOM diff."}
        ]
        error_pipeline = [
            "Unhandled Exception thrown in Controller, Service, or Policy",
            "Laravel Exception Handler (app/Exceptions/Handler.php) intercepts",
            "Exception mapped to HTTP status code (400 Bad Request, 401 Unauthorized, 403 Forbidden, 404 Not Found, 422 Validation Error, 500 Internal Error)",
            "Formatted JSON response or Filament error notification displayed to user"
        ]
        tenant_note = "Multi-tenant / User scope enforced via Filament panel guards, customer ownership relations, and global Eloquent query scopes."
...
def init_architecture(target_root=None):
    ...
    fw_info = {'express':{'language':'TypeScript','framework':'Express.js'},
               ...
               'laravel':{'language':'PHP 8.3',   'framework':'Laravel 13 & Filament v5'},
               'unknown':{'language':'PHP 8.3',   'framework':'Laravel 13 & Filament v5'}}.get(fw,{'language':'PHP 8.3','framework':'Laravel 13 & Filament v5'})
    ...
    if fw == 'laravel':
        ...
        modules = _scan_laravel(root) or _scan_express(root)
